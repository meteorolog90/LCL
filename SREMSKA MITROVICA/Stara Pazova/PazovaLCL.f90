...
open (unit = 14, file = "StaraPazova.csv", status="old", action = "read")
open (unit = 15, file = "PazovaLCL.csv")


  do i = 1, N

  read (14,*) tzsr(i),tzmin(i),tzmax(i),padavine(i),battery(i),leafWet(i),water(i),&
...
!print *, i, mpvptsr(i),ppvptsr(i),  qqsr(i)
...
print *, i, mpvptsr(i),ppvptsr(i),  qqsr(i)
  ...
  write (15, FMT="(3(F10.2,5x))") Xrta

  end do
end program PazovaLCL
