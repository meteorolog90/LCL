...
integer,dimension(N) :: god,mesec,dan,sat,battery
      !tz - temperatura zemljista
      !rvl - relativna vlaznost
      !tw - tacka rose


open (unit = 14, file = "Bogaras.csv", status="old", action = "read")
...
read (14,*) god(i),mesec(i),dan(i),sat(i),tzsr(i),tzmin(i),tzmax(i),padavine(i),battery(i),leafWet(i),tsr(i),tmin(i),&
              tmax(i),rvl(i),twsr(i),twmin(i)
